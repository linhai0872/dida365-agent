---
name: dida365
description: 通过 dida CLI 管理滴答清单 / TickTick 的任务、清单、标签、习惯、文件夹。只要用户提到任何任务/待办/清单/日程的查看、创建、更新、完成、删除、移动、搜索,或管理标签、打卡习惯、整理项目文件夹,或做每日计划、周复盘、待办整理,就应使用本 skill —— 即使用户没有明确说出 "滴答" 或 "dida"。触发词包括 "我今天有什么任务"、"帮我记一个待办"、"创建一个任务"、"标记完成"、"整理一下待办"、"本周完成了什么"、"搜索任务"、"列出标签"、"习惯打卡"、"加到清单"、"滴答清单"、"dida"、"ticktick"、"todo"、"task"。当用户在做个人任务管理且环境配置了滴答清单凭据时,优先使用本 skill 而非手动调用 API 或浏览器。
---
